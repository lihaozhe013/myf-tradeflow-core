  ...
  dbInstance.all(
    ...
    (err: Error | null, columns: ColumnInfo[]) => {
      if (err) {
        ...
        dbInstance.run(
          "ALTER TABLE partners ADD COLUMN code TEXT UNIQUE",
          (err2: Error | null) => {
            if (err2) {
  ...
  dbInstance.all(
    ...
    (err: Error | null, columns: ColumnInfo[]) => {
      if (err) {
        ...
        const needsSupplierCode = !columns.some(
          (col) => col.name === "supplier_code"
        );
        const needsProductCode = !columns.some(
          (col) => col.name === "product_code"
        );
        const needsTotalPrice = !columns.some(
          (col) => col.name === "total_price"
        );

        if (needsSupplierCode) {
          dbInstance.run(
            "ALTER TABLE inbound_records ADD COLUMN supplier_code TEXT",
            (err2: Error | null) => {
              if (err2) {
                console.error(
                  "❌ 添加 inbound_records.supplier_code 字段失败:",
                  err2.message
                );
              }
            }
        ...
        if (needsProductCode) {
          dbInstance.run(
            "ALTER TABLE inbound_records ADD COLUMN product_code TEXT",
            (err3: Error | null) => {
              if (err3) {
                console.error(
                  "❌ 添加 inbound_records.product_code 字段失败:",
                  err3.message
                );
              }
            }
        ...
        if (needsTotalPrice) {
          dbInstance.run(
            "ALTER TABLE inbound_records ADD COLUMN total_price REAL",
            (err4: Error | null) => {
              if (err4) {
                console.error(
                  "❌ 添加 inbound_records.total_price 字段失败:",
                  err4.message
                );
              } else {
            ...
            `,
                  (updateErr: Error | null) => {
                    if (updateErr) {
                      console.error(
                        "❌ 更新 inbound_records.total_price 值失败:",
                        updateErr.message
                      );
                    } else {
  ...
  dbInstance.all(
    ...
    (err: Error | null, columns: ColumnInfo[]) => {
      if (err) {
        ...
        const needsCustomerCode = !columns.some(
          (col) => col.name === "customer_code"
        );
        const needsProductCode = !columns.some(
          (col) => col.name === "product_code"
        );
        ...
        if (needsCustomerCode) {
          dbInstance.run(
            "ALTER TABLE outbound_records ADD COLUMN customer_code TEXT",
            (err2: Error | null) => {
              if (err2) {
                console.error(
                  "❌ 添加 outbound_records.customer_code 字段失败:",
                  err2.message
                );
              }
            }
        ...
        if (needsProductCode) {
          dbInstance.run(
            "ALTER TABLE outbound_records ADD COLUMN product_code TEXT",
            (err3: Error | null) => {
              if (err3) {
                console.error(
                  "❌ 添加 outbound_records.product_code 字段失败:",
                  err3.message
                );
              }
            }
        ...
        if (needsTotalPrice) {
          dbInstance.run(
            "ALTER TABLE outbound_records ADD COLUMN total_price REAL",
            (err4: Error | null) => {
              if (err4) {
                console.error(
                  "❌ 添加 outbound_records.total_price 字段失败:",
                  err4.message
                );
              } else {
            ...
            `,
                  (updateErr: Error | null) => {
                    if (updateErr) {
                      console.error(
                        "❌ 更新 outbound_records.total_price 值失败:",
                        updateErr.message
                      );
                    } else {
                      console.log(
                        "✅ 更新 outbound_records.total_price 值成功"
                      );
                    }
                  }
                );
