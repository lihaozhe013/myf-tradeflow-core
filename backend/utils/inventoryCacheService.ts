...
async function calculateAllInventory(): Promise<InventoryCacheData> {
  ...
  allModels.forEach((model) => {
    ...
    const total = decimalCalc.toDbNumber(
      decimalCalc.subtract(inQty, outQty),
      0,
    );
  ...
  calculateAllInventory()
    .then((data) => {
      try {
        ...
        logger.info(
          `Inventory cache updated. Total products: ${Object.keys(data.products).length}`,
        );
  ...
  getAllInventoryData((err, productsMap) => {
    ...
    if (filterModel) {
      ...
      items = items.filter((i) =>
        i.product_model.toLowerCase().includes(lower),
      );
    }
