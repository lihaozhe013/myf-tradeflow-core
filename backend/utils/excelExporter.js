  ...
  async exportAnalysis(options = {}) {
    try {
      ...
      if (analysisData) {
        ...
        const summaryWorksheet = this.createWorksheet(summaryData, TEMPLATES.analysis_summary);
      ...
      if (detailData && detailData.length > 0) {
        ...
        if (hasSpecificCustomer && !hasSpecificProduct) {
          // 指定客户，显示该客户的各产品销售情况
          template = TEMPLATES.analysis_detail_by_product;
          ...
          // 指定产品，显示该产品的各客户销售情况
          template = TEMPLATES.analysis_detail_by_customer;
          formattedDetailData = detailData.map(item => ({
          ...
          template = TEMPLATES.analysis_detail;
          formattedDetailData = detailData.map(item => ({
            customer_code: item.customer_code || '',
            customer_name: item.customer_name || '',
            product_model: item.product_model || '',
  ...
  async createCustomerSheets(workbook, startDate, endDate) {
    ...
    for (const customer of customerData) {
      if (customer.sales_amount > 0) { // 只导出有销售额的客户
        ...
        const summaryWorksheet = this.createWorksheet(summaryData, TEMPLATES.analysis_customer_summary);
        ...
        if (customer.product_details && customer.product_details.length > 0) {
          ...
          if (detailData.length > 0) {
            const detailWorksheet = this.createWorksheet(detailData, TEMPLATES.analysis_customer_detail);
  ...
  async createProductSheets(workbook, startDate, endDate) {
    ...
    for (const product of productData) {
      if (product.sales_amount > 0) { // 只导出有销售额的产品
        ...
        const summaryWorksheet = this.createWorksheet(summaryData, TEMPLATES.analysis_product_summary);
        ...
        if (product.customer_details && product.customer_details.length > 0) {
          ...
          if (detailData.length > 0) {
            const detailWorksheet = this.createWorksheet(detailData, TEMPLATES.analysis_product_detail);
