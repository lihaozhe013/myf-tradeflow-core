...
export default class PayableQueries {
  getPayableSummary(filters: any = {}): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      const params: any[] = [];
      if (filters.outboundFrom) { conditions.push('i.inbound_date >= ?'); params.push(filters.outboundFrom); }
      if (filters.outboundTo) { conditions.push('i.inbound_date <= ?'); params.push(filters.outboundTo); }
      if (filters.paymentFrom) { conditions.push('(p.pay_date IS NULL OR p.pay_date >= ?)'); params.push(filters.paymentFrom); }
      if (filters.paymentTo) { conditions.push('(p.pay_date IS NULL OR p.pay_date <= ?)'); params.push(filters.paymentTo); }
      ...
      db.all(sql, params, (err: any, rows: any[]) => {
        ...
        const processed = (rows || []).map(row => {
          ...
          return { ...row, total_purchases: totalPurchases, total_payments: totalPayments, balance };
        });
        resolve(processed);
      });
    });
  ...
  getPayableDetails(filters: any = {}): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      `;
      const params: any[] = [];
      if (filters.outboundFrom) { sql += ' AND inbound_date >= ?'; params.push(filters.outboundFrom); }
      if (filters.outboundTo) { sql += ' AND inbound_date <= ?'; params.push(filters.outboundTo); }
  ...
  getPayablePayments(filters: any = {}): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      `;
      const params: any[] = [];
      if (filters.paymentFrom) { sql += ' AND pay_date >= ?'; params.push(filters.paymentFrom); }
      if (filters.paymentTo) { sql += ' AND pay_date <= ?'; params.push(filters.paymentTo); }
