...
export default class AnalysisExporter {
  ...
  constructor(templates: any) { this.templates = templates; }
  ...
  exportAnalysis(options: any = {}): Buffer {
    ...
    if (analysisData) {
      const summaryData = [
        { metric_name: '销售额', amount: `¥${Number(analysisData.sales_amount || 0).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, remark: `时间: ${startDate} 至 ${endDate}` },
        { metric_name: '成本', amount: `¥${Number(analysisData.cost_amount || 0).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, remark: `客户: ${customerCode || '全部'}, 产品: ${productModel || '全部'}` },
        { metric_name: '利润', amount: `¥${Number(analysisData.profit_amount || 0).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, remark: `更新时间: ${analysisData.last_updated || ''}` },
        { metric_name: '利润率', amount: `${Number(analysisData.profit_rate || 0).toFixed(2)}%`, remark: '基于加权平均成本法计算' }
      ];
