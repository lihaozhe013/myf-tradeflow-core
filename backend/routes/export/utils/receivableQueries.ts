...
export default class ReceivableQueries {
  getReceivableSummary(filters: any = {}): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      const params: any[] = [];
      if (filters.outboundFrom) { conditions.push('o.outbound_date >= ?'); params.push(filters.outboundFrom); }
      if (filters.outboundTo) { conditions.push('o.outbound_date <= ?'); params.push(filters.outboundTo); }
      ...
      db.all(sql, params, (err: any, rows: any[]) => {
        ...
        const processed = (rows || []).map(row => {
          ...
          return { ...row, total_sales: totalSales, total_payments: totalPayments, balance };
        });
        resolve(processed);
      });
    });
  ...
  getReceivableDetails(filters: any = {}): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      `;
      const params: any[] = [];
      if (filters.outboundFrom) { sql += ' AND outbound_date >= ?'; params.push(filters.outboundFrom); }
      if (filters.outboundTo) { sql += ' AND outbound_date <= ?'; params.push(filters.outboundTo); }
