...
export default class TransactionExporter {
  ...
  exportInboundOutbound(data: any, options: any = {}): Buffer {
    ...
    if (options.tables?.includes("1") && data.inbound) {
      const worksheet = ExportUtils.createWorksheet(
        data.inbound,
        this.templates.inbound
      );
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        this.templates.inbound.sheetName
      );
    }
    if (options.tables?.includes("2") && data.outbound) {
      const worksheet = ExportUtils.createWorksheet(
        data.outbound,
        this.templates.outbound
      );
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        this.templates.outbound.sheetName
      );
    }
  ...
  exportStatement(data: any, options: any = {}): Buffer {
    ...
    if (options.tables?.includes("1") && data.inbound) {
      const worksheet = ExportUtils.createWorksheet(
        data.inbound,
        this.templates.inbound_statement
      );
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        this.templates.inbound_statement.sheetName
      );
    }
    if (options.tables?.includes("2") && data.outbound) {
      const worksheet = ExportUtils.createWorksheet(
